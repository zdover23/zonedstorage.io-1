...
The zone model of the disk can be verified by checking the *zoned* sysfs
attribute.
...
The kernel messages also include useful information about the disk.
...
In the above kernel ring buffer messages, the line "sd 5:0:0:0: [sdb]
Host-managed zoned block device" confirms that the zone model of the disk is
"Host-managed".

The total number of zones on the disk is also displayed. In this example, the
disk capacity is 15 TB and it has 55880 zones.

The zone size of the disk can also be inspected with sysfs, by checking the
...
reports the total number of zones of the disk.
