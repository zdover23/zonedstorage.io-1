...
2. The ***scsi-debug*** kernel block device driver: this driver is also simple
   to use but as its name implies, it only allows creating emulated SCSI ZBC
   hard disks.

3. The ***tcmu-runner*** SCSI device emulation: this application uses a regular
   file as a storage backstore to emulate both host-aware and host-managed ZBC
   SCSI disks. The disks created using *tcmu-runner* are handled by the Linux
   kernel like physical disks.

4. The ***QEMU*** open source machine emulator and virtualizer: recent versions
   of QEMU support the emulation of NVMe Zoned Namespaces devices that use
   regular files on the host as a backstores. Within a guest VM, the emulated
   ZNS device act like a physical device.
...
devices. Because memory backup has been added to the *null_blk* device for data
reading and writing operations, the *null_blk* driver has become a practical,
powerful tool that can be used for application development and tests.
...
specify `zoned=1` as an argument that follows the *modprobe **null_blk* command
...
method for creating emulated zoned block devices. The *configfs* parameters of
the *null_blk* driver can be listed by running the following commands:
